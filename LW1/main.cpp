...
const uint64_t NUM_OF_ELEMENTS_IN_SUM = 1000000000;
...
int main()
{
	...
	fprintf(output_file2, "array scalar; array simd; array scalar paral; array simd paral;"
		"vector scalar; vector simd; vector scalar paral; vector simd paral\n");
	for (int q = 1; q <= 10; q++)
	{
...
void run_sum_calculation(FILE* file)
{
	...
	{
		thrs1.push_back(move(thread(run_sum_loop_parallel, ref(sum_parts[i]), 
			NUM_OF_ELEMENTS_IN_SUM / processor_count * i, NUM_OF_ELEMENTS_IN_SUM / processor_count * (i + 1))));
	}
...
void run_multiplication_vectors(FILE* file)
{
	...
	for (size_t i = 0; i < processor_count; i++)
	{
		thrs1.push_back(move(thread(scalar_matr_mult_transposed_vectors_parallel, 
			ref(A), ref(B), ref(scalar_result_par), N / processor_count * i, N / processor_count * (i + 1), N)));
	}
	for (std::thread& th : thrs1)
	{
		if (th.joinable()) th.join();
	...
	for (size_t i = 0; i < processor_count; i++)
	{
		thrs2.push_back(move(thread(vector_matr_mult_transposed_vectors_parallel, 
			ref(A), ref(B), ref(vector_result_par), N / processor_count * i, N / processor_count * (i + 1), N)));
	}
	...
	fprintf(file, "%lld;%lld;%lld;%lld\n", duration1.count(), 
		duration2.count(), duration3.count(), duration4.count());

	cout << "Results are equal: " << 
		matrices_are_equal_vectors(scalar_result, vector_result, scalar_result_par, vector_result_par, N) << "\n\n";
